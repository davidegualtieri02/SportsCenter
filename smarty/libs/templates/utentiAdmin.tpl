      ...
      <div class="box">
        <div class="detail-box">
          {foreach $listaUtenti as $utente}
              <p><strong>ID utente:</strong> {$utente.id_utenteRegistrato}; <strong>Nome:</strong> {$utente.nome}; <strong>Cognome:</strong> {$utente.cognome}; <strong>Email:</strong> {$utente.email}.<br> 
              {if $utente.ban == 1}
                Utente bannato; 
              {else}
                Utente non bannato; 
              {/if}
              {if $utente.id_tessera == 0}
                Utente non tesserato.
              {else}
                Utente con ID tessera: {$utente.id_tessera}.
              {/if}
                <br>
              {if $utente.ban == 0}
                <a class="btn btn-primary" href="/SportsCenter/Amministratore/banUtente/{$utente.id_utenteRegistrato}" id="tesseramento-button">Banna l'utente</a>
              {else}
                <a class="btn btn-primary" href="/SportsCenter/Amministratore/banUtente/{$utente.id_utenteRegistrato}" id="tesseramento-button">Sbanna l'utente</a>
              {/if}
            </p><br>
          {/foreach}
        </div>
