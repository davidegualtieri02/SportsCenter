      ...
      <div class="box">
        <div class="detail-box">
          {foreach $listaPrenotazioni as $prenotazione}
              ID prenotazione: <strong>{$prenotazione.id_prenotazione}</strong>; Data: <strong>{$prenotazione.data}</strong>; Orario: <strong>{$prenotazione.orario}:00</strong>.<br>
              <p>ID utente: <strong>{$prenotazione.id_utenteRegistrato}</strong>; Nome: <strong>{$prenotazione.nomeUtente}</strong>; Cognome: <strong>{$prenotazione.cognomeUtente}</strong>; Email: <strong>{$prenotazione.emailUtente}</strong>.<br> 
              {if $prenotazione.id_tessera == 0}
                Utente non tesserato.
              {else}
                Utente con ID tessera: {$prenotazione.id_tessera}.
              {/if}<br>
              Campo: <strong>{$prenotazione.titoloCampo}</strong>
              <br>
                <a class="btn btn-primary" href="/SportsCenter/Amministratore/annullaPrenotazione/{$prenotazione.id_prenotazione}/{$prenotazione.id_utenteRegistrato}" id="tesseramento-button">Annulla prenotazione</a>
            </p><br>
          {/foreach}
        </div>
      </div>
